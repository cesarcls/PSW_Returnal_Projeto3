...
def redirect_link(request, token):
    ...
    uniques_cliques = Clicks.objects.filter(link=link).values('ip').distinct().count()
    if link.max_uniques_cliques and uniques_cliques >= link.max_uniques_cliques:
        return 404, {'error': 'Link expirado'}
...
@shortener_router.get('statatistics/{link_id}/', response={200: dict})
